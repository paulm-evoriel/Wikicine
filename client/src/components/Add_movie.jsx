  ...
  return (
    <div className="w-full mb-2 relative">
      ...
      <input
        ...
        onFocus={(e) => {
          if (inputValue.length > 1) setShow(true);
        }}
      ...
      {show && (results.length > 0 || allowAdd) && (
        <div className="absolute left-0 right-0 bg-base-100 border border-base-300 rounded-lg shadow-lg z-50 max-h-60 overflow-auto mt-1">
          ...
          {allowAdd &&
            ...
            !loading &&
            !results.some(
              (r) => r[valueKey]?.toLowerCase() === inputValue.toLowerCase()
            ) && (
              <div
                className="px-4 py-2 text-primary cursor-pointer hover:bg-base-200"
                onClick={() => {
                  onAddNew(inputValue);
                  setShow(false);
                }}
...
export default function AddMovie({ open, onClose, user }) {
  ...
  const addStudio = (studio, role) =>
    setStudios([...studios, { studio, role }]);
  ...
  const addActor = (actor, role, character_name) =>
    setActors([...actors, { actor, role, character_name }]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      setError(
        "Merci de remplir tous les champs obligatoires et d'être connecté."
      );
      return;
    }
    setError("");
  ...
  return (
    ...
    >
      <div
        className="bg-base-100 p-10 rounded-2xl shadow-2xl w-full max-w-4xl mx-4 relative overflow-y-auto max-h-[95vh] min-h-[70vh]"
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-4 text-center font-poppins">
          Ajouter un film
        </h2>
        <p className="text-base-content/70 text-center mb-6 max-w-2xl mx-auto">
          Merci de remplir tous les champs obligatoires (
          <span className="text-primary">*</span>) pour ajouter un film à la
          base de données. <br />
          <b>Titre du film</b>, <b>Date de sortie</b>, <b>Durée</b>,{" "}
          <b>Langue</b>, <b>Pays</b>, <b>Genres</b>, <b>Studios</b>,{" "}
          <b>Réalisateurs</b>, <b>Acteurs</b> et <b>Affiche</b> sont
          obligatoires.
          <br />
          Les autres champs sont facultatifs mais permettent d'enrichir la fiche
          du film (synopsis, budget, box office, identifiants, etc).
          <br />
          Utilisez les champs de recherche pour ajouter des genres, studios,
          réalisateurs ou acteurs existants, ou créez-en de nouveaux si besoin.
        </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="relative mb-2">
                ...
                {title && (
                  <button
                    type="button"
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-xl text-base-content/50 hover:text-error"
                    onClick={() => setTitle("")}
                  >
                    ×
                  </button>
              ...
              <div className="relative mb-2">
                ...
                {synopsis && (
                  <button
                    type="button"
                    className="absolute right-2 top-2 text-xl text-base-content/50 hover:text-error"
                    onClick={() => setSynopsis("")}
                  >
                    ×
                  </button>
            ...
            <div>
              ...
              {posterFile && (
                <button
                  type="button"
                  className="mt-2 btn btn-xs btn-outline btn-error"
                  onClick={() => {
                    setPosterFile(null);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {genres.map((g, i) => (
                  <span key={i} className="badge badge-primary gap-2">
                    {g.name}{" "}
                    <button
                      type="button"
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <Autocomplete
                  ...
                  fetchUrl="/studios"
                  onSelect={(s) => {
                    addStudio(s, studioRoleInput);
                    setStudioInput("");
                  }}
                  valueKey="name"
                  allowAdd={true}
                  onAddNew={(name) => {
                    addStudio({ id: null, name }, studioRoleInput);
                    setStudioInput("");
                  }}
                ...
                <select
                  ...
                  value={studioRoleInput}
                  onChange={(e) => setStudioRoleInput(e.target.value)}
                >
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <Autocomplete
                  ...
                  allowAdd={true}
                  onAddNew={(name) => {
                    addDirector({ id: null, first_name: name, last_name: "" });
                    setDirectorInput("");
                  }}
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <Autocomplete
                  ...
                  fetchUrl="/actors"
                  onSelect={(a) => {
                    addActor(a, actorRoleInput, actorCharInput);
                    setActorInput("");
                    setActorCharInput("");
                  }}
                  valueKey="last_name"
                  ...
                  onAddNew={(name) => {
                    addActor(
                      { id: null, first_name: name, last_name: "" },
                      actorRoleInput,
                      actorCharInput
                    );
                    setActorInput("");
                ...
                <input
                  ...
                  value={actorCharInput}
                  onChange={(e) => setActorCharInput(e.target.value)}
                />
              </div>
              <div className="flex flex-col gap-2">
                {actors.map((a, i) => (
